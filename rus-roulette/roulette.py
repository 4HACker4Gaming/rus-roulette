...
def roulette():
    global lv, final, item_c, item_u, dev, store, showData, oneRound, higherDealerHealth, dmg, gm, player_hp, dealer_hp, blank_a, round_a
    ...
    def dif_gm():
        global dev, dif, dif_s, showData, oneRound, higherDealerHealth, dmg, gm, canDie
...
3 for dual wield, 4 for high dealer health, or 5 for ALL of them: ''')
        if gm_c == '1':
            showData = False
    ...
    def custom_setup():
        ...
        canDie = input('Become immortal? (y/n): ') != 'y'

    dif_gm() if not custom else custom_setup()
...
{round_a} round(s).''') if showData else print('STATS OFF (Unknown stats on)')
    ...
    def d_round():
        print()
        global player_hp, dealer_hp, blank_a, round_a
